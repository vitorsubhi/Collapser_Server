...
function onSay(cid, words, param, channel)
        ...
        if not param then
			return doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_RED, "Example: /rename old_nick, new_nick")
        end
        if param2:len() <= 2 or param2:len() > 20 then
			return doPlayerSendTextMessage(cid,25,'Please 3-20 characters.')
        end
        if param2:match('[%a%d%s%u%l]+') ~= param2 then
			return doPlayerSendTextMessage(cid,25,'Please only A-Z 0-9')
        end
        ...
        if p2 and isPlayer(p2) then
			...
			addEvent(function(id,param2)
					db.query("UPDATE `players` SET `name` =  '"..param2.."' WHERE `id` ="..id)
			end,2100,id,param2)
        else
			return doPlayerSendTextMessage(cid,25,'Player '..param..' not found.')
        end
        return TRUE
end 