...
function addFragsByGuild(GuildName,amount)
	db.query("UPDATE `guilds` SET `frags` = "..getFragsByGuild(GuildName).."+"..amount.." WHERE `guilds`.`id` = "..getGuildId(GuildName)) 
end
...
function setFragsByGuild(GuildName,value)
	db.query("UPDATE `guilds` SET `frags` = "..value.." WHERE `guilds`.`id` = "..getGuildId(GuildName)) 
end
...
function getAcessTime(GuildName)
	...
	if query:getID() ~= -1 then
		return query:getDataInt("acesstime")
	end
end
...
function addAcess(GuildName, days)
	if days > 0 then
		...
		return setAcessTime(GuildName, time)
	end
	return nil 
end
...
function doRemoveAcess(GuildName, days)
	if days > 0 then
		...
		return setAcessTime(GuildName, (time <= 0 and 1 or time))
	end
	return nil 
end

function getAcessDate(GuildName)
	if HaveAcess(GuildName) then
		return os.date("%d/%m/%y %X", getAcessTime(GuildName))
	end
	return FALSE
end
