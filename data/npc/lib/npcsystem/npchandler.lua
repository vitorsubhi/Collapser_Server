...
if(NpcHandler == nil) then
	...
	function NpcHandler:onBuy(cid, itemid, subType, amount, ignore, inBackpacks)
		local callback = self:getCallback(CALLBACK_ONBUY)
		if(callback == nil or callback(cid, itemid, subType, amount, ignore, inBackpacks)) then
			if(self:processModuleCallback(CALLBACK_ONBUY, cid, itemid, subType, amount, ignore, inBackpacks)) then
				--
			end
		end
	end
	...
	function NpcHandler:onSell(cid, itemid, subType, amount, ignore, inBackpacks)
		local callback = self:getCallback(CALLBACK_ONSELL)
		if(callback == nil or callback(cid, itemid, subType, amount, ignore, inBackpacks)) then
			if(self:processModuleCallback(CALLBACK_ONSELL, cid, itemid, subType, amount, ignore, inBackpacks)) then
				--
			end
		end
	end
